function [time, e_mean, e_max] = run_gssa_experiment(RM_val, penalty_val, normalize_val)
% Solves the 1-agent growth model using GSSA with specified parameters.
% This is a modified version of Main_GSSA_1.m, designed to be called
% as a function to facilitate batch experiments.
%
% INPUTS:
%   RM_val:        Regression method (RM) code (2-8)
%   penalty_val:   Penalty value (e.g., -7 for 10^-7 eta, 7 for 10^7 kappa)
%   normalize_val: 1 to normalize, 0 to not normalize
%
% OUTPUTS:
%   time:   CPU time for the D=5 polynomial solution
%   e_mean: Mean Euler error (log10) for D=5
%   e_max:  Max Euler error (log10) for D=5
%
% -------------------------------------------------------------------------
...
% ------------------------------------ 
RM    = RM_val;       % Choose a regression method: 
                 % 1=OLS,          2=LS-SVD,   3=LAD-PP,  4=LAD-DP, 
                 % 5=RLS-Tikhonov, 6=RLS-TSVD, 7=RLAD-PP, 8=RLAD-DP
normalize = normalize_val;   % Option of normalizing the data; 0=unnormalized data; 
                 % 1=normalized data                    
penalty = penalty_val;     % Degree of regularization for a regularization methods, 
...
% --------------------------------------------------------------------------
   ...
   for D = 1:D_max
      ...
      % (in log10)
                ...
                % --- BEGIN MODIFICATION FOR FUNCTION OUTPUT ---
                % Assign the final results (for D=D_max) to the function's output
                
                time   = time_GSSA(D_max);
                e_mean = Errors_mean(D_max);
                e_max  = Errors_max(D_max);
